...
export const getEmployeeByEmployeeId = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const employee = await UserModel.findByEmployeeId(employeeId as string);
    if (!employee) {
    ...
    if (req.user && selfOnlyRoles.includes(req.user.role) && req.user.employeeId !== employeeId) {
...
export const createEmployee = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!req.user?.role || (!isSuperAdmin(req) && !canCreate.includes(req.user.role))) {
    ...
    if (!employee_id || !email || !password || !first_name || !last_name) {
      res.status(400).json({ success: false, message: 'Missing required fields: employee_id, email, password, first_name, last_name' });
      return;
    }
    ...
    const existingByEmpId = await UserModel.findByEmployeeId(employee_id);
    if (existingByEmpId) {
    ...
    res.status(201).json({ success: true, message: 'Employee created successfully', employee: employeeWithoutPassword });
  } catch (error: any) {
...
export const updateEmployee = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const updatedEmployee = await UserModel.update(employee.id, updates);
...
export const deleteEmployee = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Delete employee
    await UserModel.delete(employee.id);
