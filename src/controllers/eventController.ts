...
export const create = async (req: Request, res: Response) => {
  ...
  const event = await eventService.create(req.body as CreateEventInput, req.user.userId);
...
export const recordParticipation = async (req: Request, res: Response) => {
  ...
  const result = await eventService.recordParticipation(id, participants, req.user.userId);
