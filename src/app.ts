...
const isOriginAllowed = (origin?: string | null) => {
  ...
  // Allow localhost origins in local development
  if (
  ...
  cors({
    origin: (origin, callback) => {
      ...
      if (process.env.NODE_ENV === "production") {
        ...
        console.error(
          `[CORS]    FRONTEND_URL env: ${process.env.FRONTEND_URL || "not set"}`,
        );
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, async () => {
      ...
      console.log(
        `   GET  /api/email-config-check - Check email configuration`,
      );
      ...
      try {
        ...
        if (status.overall) {
          ...
          console.error(
            `   Set EMAILJS_SERVICE_ID, PUBLIC_KEY, and PRIVATE_KEY in .env`,
          );
        }
      } catch (error: any) {
        console.error(
          `📧 Email Configuration: ⚠️  Could not check configuration: ${error.message}`,
        );
      }
    });
  } catch (error) {
