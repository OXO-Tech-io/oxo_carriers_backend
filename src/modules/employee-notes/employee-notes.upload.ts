...
// Mirrors src/middleware/upload.ts's `uploadNoteAttachments` exactly (same
// storage/fileFilter/limits, field name 'attachments', up to 5 files) - that
// file is still used by the old Express stack and must not be edited, so the
// equivalent multer options are reproduced here for the Nest FilesInterceptor.
...
export const noteAttachmentsMulterOptions: MulterOptions = {
  storage,
  fileFilter,
