...
vi.mock("../../src/modules/employee-nominees/EmployeeNominee", () => ({
  EmployeeNomineeModel: { listByEmployeeId: vi.fn() },
}));
...
import { EmployeeNomineesService } from "../../src/modules/employee-nominees/employee-nominees.service";
import { EmployeeNomineesController } from "../../src/modules/employee-nominees/employee-nominees.controller";

const listMock = EmployeeNomineeModel.listByEmployeeId as unknown as ReturnType<typeof vi.fn>;
const findByIdMock = EmployeeModel.findById as unknown as ReturnType<typeof vi.fn>;

describe("EmployeeNomineesService", () => {
  const service = new EmployeeNomineesService();
  ...
  it("lists another employee's records for HR/admin roles via query param", async () => {
    ...
    const result = await service.list("EMPHR", UserRole.HR_EXECUTIVE, "EMP2");
  ...
  it("throws BadRequestException for HR/admin roles missing the query employeeId", async () => {
    await expect(service.list("EMPHR", UserRole.HR_EXECUTIVE, undefined)).rejects.toThrow(BadRequestException);
  });
});

describe("EmployeeNomineesController", () => {
  let service: EmployeeNomineesService;
  let controller: EmployeeNomineesController;

  beforeEach(() => {
    vi.clearAllMocks();
    service = new EmployeeNomineesService();
    controller = new EmployeeNomineesController(service);
  });
  ...
  it("returns a success envelope with the nominee records", async () => {
    findByIdMock.mockResolvedValue({ employeeId: "EMP1" });
    listMock.mockResolvedValue([{ id: 1, nameWithInitials: "J. Doe" }]);
    const result = await controller.list(1, { employeeId: "EMP1", role: UserRole.EMPLOYEE } as any);
    expect(result).toEqual({
      success: true,
      message: "Nominee records fetched",
      data: [{ id: 1, nameWithInitials: "J. Doe" }],
    });
  });
});
